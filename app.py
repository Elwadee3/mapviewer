...
# -------------------------
# تحسين المظهر باستخدام CSS المطور (إخفاء تام للأزرار الخفية)
# -------------------------
st.markdown("""
<style>
    ...
    /* إخفاء تام ومطلق لنص وأبعاد أزرار الاختيار خلف البطاقات */
    div.stButton > button {
        border: none !important;
        background: transparent !important;
        color: transparent !important; /* جعل الخط شفاف تماماً */
        text-shadow: none !important;
        font-size: 0px !important;     /* تصفير حجم الخط لضمان عدم ظهوره */
        line-height: 0 !important;
        padding: 0 !important;
        margin: 0 !important;
        height: 0px !important;        /* إلغاء الارتفاع لتفادي الدفع لأسفل */
        width: 100% !important;
    ...
    /* تصميم مظهر البطاقات الجانبية بنقاوة كاملة */
    .control-card {
...
def create_graph(selected_id, source_text, mappings):
    ...
    # الحفاظ على الحجم الضخم الممتاز للدائرة المركزية الزرقاء (160)
    net.add_node(
        selected_id, 
        label=selected_id, 
...
if os.path.exists(DATA_FILE):
    ...
    if filtered_controls:
        for ctrl_id in filtered_controls:
            ...
            with st.sidebar:
                # تعديل جذري: تم تفريغ النص هنا تماماً وتصفير أبعاده بالـ CSS لمنع أي تسريب بصري
                if st.button(label="", key=f"btn_{ctrl_id}"):
                    st.session_state.selected_control_id = ctrl_id
                    st.rerun()
                
                # طباعة المظهر الفعلي للبطاقة بمحاذاة علوية مثالية بدون زحزحة
                st.markdown(f"""
                <div class="{card_class}" style="margin-top: 0px; pointer-events: none;">
...
def create_graph(selected_id, source_text, mappings):
    ...
    net.add_node(
        ...
        title=html.escape(source_text), 
        color="#1687d9", 
        size=200,  # تم التكبير بشكل ملحوظ بناءً على طلبك لتبدو ضخمة ومتناسقة
        shape="circle", 
        font={'color': 'white', 'size': 32, 'bold': True}
    )
